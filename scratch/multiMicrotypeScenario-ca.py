...
for mode in ["auto", "bike", "bus", "rail", "walk"]:
    ...
    if 'PerStartCost' not in df.columns:
        df['PerStartCost'] = 0.0
    newdf = df.loc[df.MicrotypeID.str[0].isin(geotypes), :]
...
df = pd.read_csv(oldPath)
newdf = df.loc[df.OriginMicrotypeID.str[0].isin(geotypes) & df.DestinationMicrotypeID.str[0].isin(geotypes), :]
newdf.sort_values(newdf.columns[0], ascending=True).to_csv(newPath, index=False)
...
df = pd.read_csv(oldPath)
newdf = df.loc[
        df.FromMicrotypeID.str[0].isin(geotypes) & df.ToMicrotypeID.str[0].isin(geotypes) & df.ThroughMicrotypeID.str[
            0].isin(geotypes), :]
newdf.sort_values(newdf.columns[0], ascending=True).to_csv(newPath, index=False)
...
df = pd.read_csv(oldPath)
newdf = df.loc[df.MicrotypeID.str[0].isin(geotypes), :].replace("hv", "auto")
...
newdf = df.loc[df.HomeMicrotypeID.str[0].isin(geotypes) & df.OriginMicrotypeID.str[0].isin(geotypes) &
               df.DestinationMicrotypeID.str[0].isin(geotypes), :]
newdf.sort_values(newdf.columns[0], ascending=True).to_csv(newPath, index=False)
...
df = pd.read_csv(oldPath)
newdf = df.loc[df.MicrotypeID.str[0].isin(geotypes), :].rename(
...
newdf = df.loc[df.index.get_level_values(0).str[0].isin(geotypes) & df.index.get_level_values(1).str[0].isin(geotypes) &
               df.index.get_level_values(3).str[0].isin(geotypes), df.columns.str[0].isin(geotypes)]
